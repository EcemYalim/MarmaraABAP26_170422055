CLASS zcl_170422055_rest_client_02 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_170422055_rest_client_02 IMPLEMENTATION.
...
*    io_out->write( user_delete(  ) ).
  ...
  METHOD user_get.
    ...
    rv_text = execute_request(
      iv_method = 'GET'
      iv_url    = gc_base_url && '/user/demo_serpil' ).
  ...
  METHOD build_user_json.
    rv_json =
        '{' &&
        '"id": 5050,' &&
        '"username": "demo_serpil",' &&
        '"firstName": "serpil22",' &&
        '"lastName": "cbnlr",' &&
        '"email": "serpil2@gmail.com.tr",' &&
        '"password": "Srpl123.",' &&
        '"phone": "5036789876",' &&
        '"userStatus": 1' &&
        '}'.
